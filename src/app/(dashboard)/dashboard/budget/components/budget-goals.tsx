...
export function BudgetGoals({ userId }: BudgetGoalsProps) {
  ...
  const handleToggleStatus = async (goal: BudgetGoal) => {
    try {
      ...
      setGoals(goals.map(g => g.id === goal.id ? { ...g, title: goal.title.split(":")[0] === "TRUE" ? "FALSE:" + goal.title.split(":")[1] : "TRUE:" + goal.title.split(":")[1] } : g))
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-gray-100">
        ...
        <div className="flex flex-col items-center justify-center h-[70vh]">
          ...
          <p className="text-gray-300">Loading your budget goals...</p>
        </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-gray-800 border-gray-700">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-gray-300">Your financial targets</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-yello">{goals.filter(g => g.title.startsWith("FALSE")).length}</div>
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg text-white">Completed Goals</CardTitle>
            <CardDescription className="text-gray-300">Your financial targets</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-green">{goals.filter(g => g.title.startsWith("TRUE")).length}</div>
            <div className="text-sm text-gray-300 mt-1">
        ...
        {/* <Card className="bg-gray-800 border-gray-700">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-gray-300">All goals combined</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg text-white">Next Deadline</CardTitle>
            <CardDescription className="text-gray-300">Upcoming goal</CardDescription>
          </CardHeader>
          <CardContent>
            {nextDeadlineGoal ? (
              <>
              ...
              <div className="text-gray-300">No upcoming goals</div>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="active">
        ...
        <TabsContent value="active">
          <div className="space-y-6">
            {goals.length === 0 ? (
              <Card className="bg-gray-800 border-gray-700">
                <CardHeader>
                  <CardTitle className="text-white">No Goals Found</CardTitle>
        ...
        <TabsContent value="add">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              ...
              <CardDescription className="text-gray-300">Set a new financial target</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="projections">
          <Card className="bg-gray-800 border-gray-700">
            <CardHeader>
              <CardTitle className="text-white">Goal Projections</CardTitle>
              <CardDescription className="text-gray-300">Your financial goals timeline</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-medium text-white">Goal Timeline</h3>
                  <div className="relative">
                    <div className="absolute top-0 bottom-0 left-[15%] w-0.5 bg-gray-600" />
                      ...
                      {goals.map((goal, index) => {
                        ...
                        return (
                          <div key={goal.id} className="relative pl-8 ml-[15%]">
                            <div className={`absolute left-0 top-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 rounded-full ${color}`} />
                            <div className="p-4 bg-gray-700 rounded-lg border border-gray-600">
                              <div className="font-medium text-white">{goal.title.split(":")[1]}</div>
    ...
    <Card className={`bg-gray-800 border-gray-700 ${isOverdue ? 'border-l-4 border-l-red-500' : ''}`}>
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          <div>
            <CardTitle className="text-white">{goal.title.split(":")[1]}</CardTitle>
            <CardDescription className="text-gray-300">
              {goal.category}
            </CardDescription>
          </div>
